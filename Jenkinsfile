pipeline {
    ...
    environment {
        IMAGE_NAME = "abakhar217/user-profile-service:user-profile-service-${BUILD_NUMBER}"
        DEPLOYMENT_NAME = 'user-profile-service-deployment'
        ...
        stage('Checkout') {
            steps {
                ...
                checkout([$class: 'GitSCM',
                          branches: [[name: 'new-version-med']],
                          userRemoteConfigs: [[url: 'https://github.com/InvestBuddy/user-profile-service.git', credentialsId: 'git']]])
            }
        }
        ...
        stage('Check if JAR exists') {
            steps {
                script {
                    ...
                    if (!fileExists('target/user-profile-service-1.0-SNAPSHOT.jar')) {
                        error "user-profile-service-1.0-SNAPSHOT.jar not found! Build failed."
                    }
                }
            }
        }
